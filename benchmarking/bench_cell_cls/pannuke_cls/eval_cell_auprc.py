"""
Cell-level AUPRC for PanNuke cell-type classification under MIPHEI's EXACT protocol
(the PanNuke analog of cell_cls/pathocell_cls/eval_cell_auprc.py).

A logreg maps per-cell predicted marker expression to the 5 PanNuke nuclei classes,
on MIPHEI's fixed 20%/80% split. Comparable to checkpoints/<model>/pannuke_logreg.csv.
...
Sources (--source):
  ...
  parquet : a MIPHEI pannuke parquet with <marker>_pred (--pred_parquet) — reproduces
            their pannuke_logreg.csv as a harness check.

Examples:
  python cell_cls/pannuke_cls/eval_cell_auprc.py --tag bg0.2
  python cell_cls/pannuke_cls/eval_cell_auprc.py --source parquet \
      --pred_parquet checkpoints/MIPHEI-vit/pannuke_cell_dataframe_logreg.parquet
"""
import argparse, re
