"""
Cell-level metric plots for PanNuke (AUPRC or F1), reading the per-class + MEAN5 rows
from eval_cell_auprc_<tag>.csv. Mirror of pathocell_cls/plot_metrics.py.

Encoding: COLOUR = model identity (a `-tokenagg` variant shares its native model's
colour), HATCH (//) = token-agg / matched aggregation, ours = blue. Same for auprc/f1.

  python cell_cls/pannuke_cls/plot_metrics.py --metric auprc
  python cell_cls/pannuke_cls/plot_metrics.py --metric auprc --compare MIPHEI-vit
"""
import argparse
...
def compare_types(d: Path, comp_tag: str, metric: str, out: str | None,
    ...
    """Per-class comparison: competitor `comp_tag` vs ours (2 bars/class). Colour = model
    ...
    fig, ax = plt.subplots(figsize=(max(9, len(types) * 1.6), 5.5))
    top_max = 0.0
    ...
    ax.set_title(f"PanNuke {NICE[metric]} per class — ours vs {comp_tag}\n"
...
def main() -> None:
    ...
    ap.add_argument("--compare", nargs="+", metavar="COMP", default=None,
                    help="per-class competitor-vs-ours plot, e.g. --compare MIPHEI-vit")
    ap.add_argument("--out", default=None)
    args = ap.parse_args()
    metric = args.metric; d = Path(args.dir)

    if args.compare:
        comp = [t for t in args.compare if t != OURS_TAG]
        if not comp:
            raise SystemExit("--compare needs a competitor tag, e.g. --compare MIPHEI-vit")
        compare_types(d, comp[0], metric, args.out); return
    ...
    if not rows:
        raise SystemExit("no MEAN5 rows — run eval_cell_auprc.py first")

    df = pd.DataFrame(rows)
    if not args.tags:
        df = df.sort_values("val", ascending=False).reset_index(drop=True)
    labels = df["tag"].tolist(); x = np.arange(len(labels))
    ...
    ax.set_title(f"PanNuke cell-type mean-5 {NICE[metric]} by model (MIPHEI protocol)", fontsize=12)
    ax.grid(axis="y", lw=0.4, alpha=0.4)
    ...
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"saved -> {out}"); print(df.to_string(index=False))
