"""
Reproduce MIPHEI's released PanNuke cell-type numbers with OUR harness.

For every MIPHEI checkpoint shipping a pannuke cell parquet + logreg CSV, run OUR
exact logreg on THEIR predictions and check we recover THEIR published per-class
AUPRC / F1 (checkpoints/<model>/pannuke_logreg.csv). If we reproduce their numbers,
the harness is unbiased — so a higher score for our model is a real edge.

Example:
  python cell_cls/pannuke_cls/reproduce_miphei.py
"""
import argparse
...
def main():
    ...
    models = sorted(d.name for d in CKPT_DIR.iterdir()
                    if (d / "pannuke_cell_dataframe_logreg.parquet").exists()
                    and (d / "pannuke_logreg.csv").exists())
    print(f"reproducing {len(models)} MIPHEI models | tol={args.tol}\n")

    summ = []
    for model in models:
        mine = eval_parquet(CKPT_DIR / model / "pannuke_cell_dataframe_logreg.parquet")
        theirs = pd.read_csv(CKPT_DIR / model / "pannuke_logreg.csv").set_index("Marker")
