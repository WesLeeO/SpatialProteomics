"""
Cell-level metric plots for Lizard (AUPRC or F1), reading the per-class + MEAN6 rows
...
native pixel-in-nucleus, ours = blue. The miphei_pixel/miphei_token ablation tags are
excluded from the default model_comparison (they're matched-res ablations, not baselines).

  python cell_cls/lizard_cls/plot_metrics.py --metric auprc
  python cell_cls/lizard_cls/plot_metrics.py --metric auprc --compare MIPHEI-vit
"""
import argparse
...
OURS_BLUE = "#1f77b4"
COMP_COLOR = "#9467bd"
EXCLUDE = {"miphei_pixel", "miphei_token"}   # matched-res ablations, not baselines
...
def compare_types(d: Path, comp_tag: str, metric: str, out: str | None,
    ...
    ax.set_title(f"Lizard {NICE[metric]} per class — ours vs {comp_tag}\n"
...
def main() -> None:
    ...
    tags = args.tags or sorted(p.stem.replace("eval_cell_auprc_", "")
                               ...
                               if p.stem.replace("eval_cell_auprc_", "") not in EXCLUDE)
    rows = []
    for t in tags:
    ...
    if not rows:
        raise SystemExit("no MEAN6 rows — run eval_cell_auprc.py first")
    ...
    ax.set_title(f"Lizard cell-type mean-6 {NICE[metric]} by model (MIPHEI protocol)", fontsize=12)
    ax.grid(axis="y", lw=0.4, alpha=0.4)
