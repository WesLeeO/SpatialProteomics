...
CORE_PNG_DIR   = Path("datasets/immunoatlas_70_png")   # 70 per-core H&E PNGs at HE_MPP
...
def compute_global_p99s(core_names, core_channels, protein_cols):
    """Pool non-zero pixels across all TMA cores to compute one p99 per channel."""
    pixel_pools = [[] for _ in protein_cols]

    for ci, core_name in enumerate(core_names):
        print(f"  [p99 pass {ci+1}/{len(core_names)}] {core_name}", flush=True)
    ...
    global_p99s = np.ones(len(protein_cols), dtype=np.float32)
    print("\nGlobal p99s:")
    for mi, cname in enumerate(protein_cols):
        if pixel_pools[mi]:
            all_vals = np.concatenate(pixel_pools[mi])
            global_p99s[mi] = float(np.percentile(all_vals, 99))
        print(f"  {cname}: {global_p99s[mi]:.1f}  ({sum(len(a) for a in pixel_pools[mi]):,} px)")
...
def compute_token_grid_targets(patch_coords, proteins, patch_size_level0,
                               global_p99s, token_grid=TOKEN_GRID):
    """
    ...
    Pipeline per patch:
      ...
      4. Normalise: clip(x / global_p99, 0, 1)
    ...
    p99s_arr = np.maximum(global_p99s, 1.0).astype(np.float32)   # (C,)
    ...
    for i, (px, py) in enumerate(sorted_coords):
        ...
        # normalise: clip(x / global_p99, 0, 1)
        normed = np.clip(resized / p99s_arr[np.newaxis, np.newaxis, :],
                         0.0, 1.0)   # (224, 224, C)
...
def save_dataset(out_path, all_coords, all_targets, all_core_ids,
                 global_p99s, mag, patch_size, patch_size_level0, protein_cols,
...
def build_patch_dataset():
    ...
    print(f"  normalisation: clip(x / global_p99, 0, 1)")
    ...
    # Pass 1: compute dataset-wide p99 per channel
    print("\n" + "=" * 60)
    print("  Pass 1: computing global p99s across all cores")
    print("=" * 60)
    global_p99s = compute_global_p99s(core_names, core_channels, protein_cols)

    # Pass 2: extract patches and normalise with global p99s
    print("\n" + "=" * 60)
    ...
    for core_idx, core_name in enumerate(core_names):
        ...
        sorted_coords, targets = compute_token_grid_targets(
            coords, proteins, patch_size_level0, global_p99s, token_grid=TOKEN_GRID,
        )
    ...
    save_dataset(
        Path(args.output), all_coords, all_targets, all_core_ids, global_p99s,
...
Channel index reference:
  0. Hoechst
  1. CD164  2. FOXP3  3. GATA3  4. MUC-1  5. p53  6. Vimentin  7. T-bet
  8. Cytokeratin  9. PD-L1  10. Ki-67  11. CD15  12. CD30  13. CD2
  14. GranzymeB  15. CD5  16. MMP-9  17. CD4  18. LAG-3  19. CD25  20. CD56
  21. CD20  22. PD-1  23. CD11c  24. CD162  25. CD16  26. CD11b  27. CD194
  28. IDO-1  29. EGFR  30. VISTA  31. HLA-DR  32. ICOS  33. BCL-2  34. CD3
  35. CD69  36. CD8  37. CD7  38. CD45RA  39. CD45  40. CD1a  41. CD57
  42. B-catenin  43. CD45RO  44. CD71  45. CD34  46. CD68  47. CD38
  48. Collagen-IV  49. CD31  50. Podoplanin  51. CD138  52. CD163
  53. Mast-cell-tryptase  54. MMP-12
"""