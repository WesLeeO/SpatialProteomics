"""
Cell-level metric plots for PathoCell (one metric at a time: AUPRC or F1), reading
the per-type + MEAN15 rows from eval_cell_auprc_<tag>.csv (written by
eval_cell_auprc.py). Mirror of orion_cell_cls/plot_metrics.py.

Encoding (same as orion): COLOUR = model identity (a `-tokenagg` variant shares its
native model's colour), HATCH (//) = token-agg / matched aggregation, ours = blue.

Two modes:
  default        one bar per MODEL for the mean-15 headline:
    python cell_cls/pathocell_cls/plot_metrics.py --metric auprc
    python cell_cls/pathocell_cls/plot_metrics.py --metric f1 --tags ours MIPHEI-vit

  --compare COMP  per-type comparison, competitor vs ours (2 bars/type):
    python cell_cls/pathocell_cls/plot_metrics.py --metric auprc --compare MIPHEI-vit
"""
import argparse
...
OURS_TAG = "ours"
OURS_BLUE = "#1f77b4"
COMP_COLOR = "#9467bd"     # single-competitor colour in --compare
...
def compare_types(d: Path, comp_tag: str, metric: str, out: str | None,
                  ours_tag: str = OURS_TAG) -> None:
    """Per-type comparison: competitor `comp_tag` vs ours (2 bars/type). Colour = model
    (ours blue, competitor purple); a -tokenagg competitor gets the diagonal hatch."""
    ...
    types = sorted([m for m in ref.index if m in common],
    ...
    # ours uses token aggregation → hatch it; competitors are native (solid) unless -tokenagg
    hatch = lambda t: "//" if (t == ours_tag or t.endswith("-tokenagg")) else ""

    fig, ax = plt.subplots(figsize=(max(11, len(types) * 1.05), 5.5))
    top_max = 0.0
    for j, (t, df) in enumerate(loaded):
        v  = df.loc[types, f"{metric}_boot_mean"].values
        lo = df.loc[types, f"{metric}_ci_lo"].values
        hi = df.loc[types, f"{metric}_ci_hi"].values
    ...
    ax.set_xticks(x); ax.set_xticklabels(types, rotation=45, ha="right", fontsize=9)
    ax.set_ylabel(f"{NICE[metric]} (bootstrap mean, 95% CI)")
    ax.set_ylim(0, min(1.08, top_max + 0.12))
    means = {t: df.loc[types, f"{metric}_boot_mean"].mean() for t, df in loaded}
    ax.set_title(f"PathoCell {NICE[metric]} per cell type — ours vs {comp_tag}\n"
...
def main() -> None:
    ...
    ap.add_argument("--tags", nargs="*", default=None,
                    help="model tags to include (default: all found, sorted by score)")
    ap.add_argument("--compare", nargs="+", metavar="COMP", default=None,
                    help="per-type competitor-vs-ours plot, e.g. --compare MIPHEI-vit")
    ...
    if args.compare:
        ...
        compare_types(d, comp[0], metric, args.out)
        return
    ...
    # COLOUR = model identity (token-agg shares native colour), HATCH = token-agg, ours = blue
    ...
    # hatch ours (token-agg) + any -tokenagg competitor; native competitors stay solid
    ...
    ax.set_title(f"PathoCell cell-type mean-15 {NICE[metric]} by model (MIPHEI protocol)",
                 fontsize=12)
    ax.grid(axis="y", lw=0.4, alpha=0.4)
    ...
    ax.add_artist(leg1)
    if any(hatches):   # only show the aggregation legend once token-agg bars exist
