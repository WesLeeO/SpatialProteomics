...
DATASET_CONFIGS = [
    ...
    dict(
        ...
        h5_dir      = "datasets/hemit_patch_dataset",
    ),
    dict(
        name    = "sg",
    ...
    dict(
        name    = "jedi",
        type    = "jedi",
        h5_path = "datasets/jedi_patch_dataset/JEDI20034_patch_dataset.h5",
        he_tif  = "/mnt/ssd1/virtual_proteomics/data/JEDI_201207/JEDI20033.tif",
    ),
    dict(
        name      = "nsclc_charite",
        type      = "jpeg_crops",
        h5_path   = "datasets/nsclc_charite_patch_dataset.h5",
    ...
    dict(
        name      = "immunoatlas",
        type      = "png_crops",
        h5_path   = "datasets/immunoatlas_NOLN210920_patch_dataset.h5",
        he_dir    = "datasets/immunoatlas_70_png",
        he_suffix = ".png",
        id_key    = "core_ids",
    ),
]
...
# ── Training config ────────────────────────────────────────────────────────────
MODEL_NAME       = "UNI2"
OUTPUT_DIR       = Path(f"training_outputs/outputs_multisource_{MODEL_NAME}")
TOKEN_GRID       = 16
...
def _hed_jitter(patch: np.ndarray, sigma: float = 0.05, bias: float = 0.05) -> np.ndarray:
    """HED colour augmentation (Tellez et al. 2018).

    patch : (H, W, 3) uint8 RGB → (H, W, 3) uint8 RGB
    Skips patches with mean intensity outside [0.05, 0.95] (background / saturated).
    """
    mean = patch.mean() / 255.0
    if not (0.05 <= mean <= 0.95):
        return patch
    hed = skimage.color.rgb2hed(patch.astype(np.float32) / 255.0)
    for c in range(3):
        hed[:, :, c] *= 1.0 + np.random.uniform(-sigma, sigma)
        hed[:, :, c] +=       np.random.uniform(-bias,  bias)
    rgb = np.clip(skimage.color.hed2rgb(hed), 0.0, 1.0)
    return (rgb * 255).astype(np.uint8)


def augment_patch(patch: np.ndarray, target: np.ndarray,
                  p_hed: float = 0.5) -> tuple[np.ndarray, np.ndarray]:
...
class MultiSourcePatchDataset(Dataset):
    ...
    def _load_he(self, item: tuple) -> np.ndarray:
        ...
        if src == "orion":
            ...
            # trim black border before resize (edge patches from circular TMA cores)
            occupied = patch.any(axis=2)
...
def run_epoch(model, loader, optimizer=None, scaler=None, scheduler=None,
    ...
    with torch.set_grad_enabled(training):
        for i, batch in enumerate(loader):
            patches, targets_cpu, mask_cpu = batch
